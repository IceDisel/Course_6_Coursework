...
def send_mailing():
    ...
    now = timezone.now()
    mailings = MailingSrv.objects.filter(status='создана', is_activated=True, next__lte=now, finish__gte=now)
    ...
    for mailing in mailings:
        ...
        log = Log.objects.create(mailing=mailing, status=status, server_response=server_response)
...
def manual_send_mailing(pk):
    ...
    result = send_mail(subject=subject, message=message, from_email=settings.EMAIL_HOST_USER,
                       recipient_list=mailing_list, fail_silently=False, )
